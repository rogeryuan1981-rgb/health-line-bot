    ...
    return (
        <div className="space-y-3 bg-slate-800/30 p-4 rounded-xl border border-white/5">
            <div className="flex justify-between items-center text-[10px] font-bold text-slate-400">
                ...
                <button onClick={() => { if(urls.length < 5) setNodeData({...nodeData, imageUrls: [...urls, '']}) }} className="text-[#deff9a]"><Plus size={14}/></button>
            </div>
            {urls.map((url: string, idx: number) => (
                <div key={idx} className="space-y-2 border-b border-white/5 pb-3 last:border-0">
                    <div className="flex justify-between items-center">
                        ...
                        <button onClick={() => setActiveLib(activeLib === `image-${idx}` ? null : `image-${idx}`)} className="text-[#deff9a] text-[10px] flex items-center gap-1"><Library size={12}/> 資源庫</button>
                    </div>
                    <div className="flex gap-2">
                        ...
                        {urls.length > 1 && <button onClick={() => { const newUrls = [...urls]; newUrls.splice(idx, 1); setNodeData({...nodeData, imageUrls: newUrls, imageUrl: newUrls[0]}); }} className="text-red-500 p-2 hover:bg-red-500/20 rounded-xl"><Trash2 size={14}/></button>}
                    </div>
      ...
      <div className="flex-1 overflow-y-auto scrollbar-hide flex flex-col">
        <div className="p-6 space-y-6">
                ...
                <label className="text-[10px] font-bold text-slate-500 uppercase tracking-widest flex items-center gap-1">
                    ...
                    <div className="group relative flex items-center">
                        ...
                        <div className="absolute left-4 top-0 w-48 bg-slate-800 text-slate-300 text-[9px] p-2.5 rounded-lg shadow-xl opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all z-50 border border-white/10 font-normal normal-case">
                            使用者輸入此字將觸發。若名為 <span className="text-[#deff9a] font-bold">預設回覆</span> 則為保底節點。
                        </div>
                    </div>
                </label>
            ...
            {(nodeData.messageType === 'flex' || nodeData.messageType === 'carousel') && (
                <div className="flex gap-2 bg-slate-900 p-1 rounded-xl animate-in slide-in-from-top-1">
            ...
            <div className="space-y-4 border-t border-white/5 pt-4">
                {nodeData.messageType === 'text' && (
                    <div className="space-y-2 animate-in fade-in">
                        ...
                        {showEmoji && (
                            <div className="grid grid-cols-8 gap-1 bg-slate-900 p-2 rounded-xl max-h-32 overflow-y-auto scrollbar-hide border border-white/5 shadow-inner">
                ...
                {nodeData.messageType === 'video' && (
                    <div className="space-y-4 animate-in fade-in">
                        <div className="space-y-2">
                          ...
                          <input value={nodeData.imageUrl || ""} onChange={e => setNodeData({...nodeData, imageUrl: e.target.value})} className="w-full bg-slate-900 border-none rounded-xl px-4 py-2 text-xs outline-none" placeholder="封面圖網址..." />
                          {activeLib==='vCover' && renderLibraryDropdown((url)=>setNodeData({...nodeData, imageUrl:url}))}
                        </div>
                        <div className="space-y-2">
                          <label className="text-[10px] font-bold text-slate-500 uppercase">影片檔案網址 (HTTPS)</label>
                          <input value={nodeData.videoUrl || ""} onChange={e => setNodeData({...nodeData, videoUrl: e.target.value})} className="w-full bg-slate-900 border-none rounded-xl px-4 py-2 text-xs outline-none" placeholder="影片網址..." />
                        </div>
                        <div className="space-y-2">
                          <label className="text-[10px] font-bold text-slate-500 uppercase">影片下方文字</label>
                          <textarea value={nodeData.textContent || ""} onChange={e => setNodeData({...nodeData, textContent: e.target.value})} className="w-full bg-slate-900 rounded-xl p-3 text-xs outline-none min-h-[60px]" placeholder="影片說明文字..." />
                        </div>
                    </div>
                )}
                ...
                {nodeData.messageType === 'file' && (
                    <div className="space-y-4 animate-in fade-in">
                        ...
                        <div className="space-y-2">
                          <label className="text-[10px] font-bold text-slate-500 uppercase">檔案名稱</label>
                    ...
                    <div className="space-y-4 animate-in fade-in">
                        ...
                        <div className="space-y-2">
                          <div className="flex justify-between items-center">
                            ...
                            <button onClick={() => setActiveLib(activeLib==='fImg'?'null':'fImg')} className="text-[#deff9a] text-[10px] flex items-center gap-1"><Library size={12}/> 資源庫</button>
                          </div>
                          <input value={nodeData.imageUrl || ""} onChange={e => setNodeData({...nodeData, imageUrl: e.target.value})} className="w-full bg-slate-900 border-none rounded-xl px-4 py-2 text-xs outline-none" placeholder="圖片網址 (https://)" />
                          {activeLib==='fImg' && renderLibraryDropdown((url)=>setNodeData({...nodeData, imageUrl:url}))}
                        </div>
                        <div className="space-y-2">
                            <label className="text-[10px] font-bold text-slate-500 uppercase">內容文字</label>
                        ...
                        <div className="space-y-3 bg-slate-800/50 p-4 rounded-xl border border-white/5">
                            ...
                            {nodeData.buttons?.map((btn: any, i: number) => (
                                <div key={i} className="flex gap-2 items-center">
                                    ...
                                    <button onClick={() => { const nb = [...nodeData.buttons]; nb.splice(i,1); setNodeData({...nodeData, buttons: nb}) }} className="text-red-500 p-1 hover:bg-red-500/10 rounded-full"><Trash2 size={12}/></button>
                                </div>
                            ))}
                        </div>
                    </div>
            ...
            <button onClick={() => { if(window.confirm("刪除節點？")) deleteDoc(doc(db, "flowRules", nodeId!)); onClose(); }} className="w-full text-red-500/50 hover:text-red-500 text-[10px] py-4 uppercase font-bold tracking-widest flex items-center justify-center gap-1 transition-colors mt-4 border-t border-white/5 pt-8"><Trash2 size={12}/> Delete Node</button>
        </div>

        <div className="p-6 bg-slate-900/40 border-t border-white/10 mt-auto">
            <h4 className="text-[10px] font-bold text-slate-500 mb-4 uppercase tracking-widest">實機預覽</h4>
            <LineSimulator data={nodeData} />
        </div>
      </div>

      <div className="p-6 border-t border-white/10 bg-slate-900 flex gap-3 z-50 shadow-[0_-10px_20px_rgba(0,0,0,0.2)]">
