  ...
  return (
    <>
        {data.messageType === 'text' && (
            <div className="bg-white rounded-xl rounded-tl-none px-3 py-2 text-xs text-slate-700 whitespace-pre-wrap shadow-sm">{data.textContent || "輸入文字..."}</div>
        )}
        ...
        {data.messageType === 'file' && (
            <div className="w-48 bg-white rounded-xl shadow-sm border border-gray-100 p-3 flex items-center gap-3"><div className="w-10 h-10 bg-slate-100 text-slate-500 rounded-lg flex items-center justify-center flex-shrink-0"><FileText size={20} /></div><div className="flex-1 overflow-hidden"><div className="text-xs font-bold text-slate-800 truncate">{data.textContent || "未命名檔案.pdf"}</div><div className="text-[10px] text-slate-400 mt-0.5">1.0 MB</div></div></div>
        )}
        {data.messageType === 'video' && (
            <div className="w-56 bg-white rounded-xl overflow-hidden shadow-sm flex flex-col border border-gray-100"><div className="relative aspect-video"><img src={data.imageUrl || defaultImg} className="w-full h-full object-cover opacity-80" alt="Preview" /><div className="absolute inset-0 flex items-center justify-center"><div className="w-10 h-10 bg-black/60 rounded-full flex items-center justify-center"><Youtube className="text-white fill-white" size={20} /></div></div></div>{data.textContent && <div className="p-3 text-[12px] text-slate-800 whitespace-pre-wrap leading-relaxed font-medium">{data.textContent}</div>}</div>
        )}
        {isFlex && (
        ...
        {isCarousel && (
            <div className="flex gap-2 overflow-x-auto pb-2 scrollbar-hide w-full max-w-[320px]">
                ...
                <div key={idx} className={`${cardWidth} bg-white rounded-xl overflow-hidden shadow-sm flex-shrink-0 border border-gray-100 flex flex-col`}>
                    ...
                    <div className="p-3 space-y-2 flex-1 flex flex-col"><div className="font-bold text-xs text-slate-800">{card.title || "卡片標題"}</div><div className="text-[10px] text-slate-500 flex-1">{card.price || "內容說明"}</div>
...
export default function LineSimulator({ data }: { data: any }) {
  ...
  return (
    <div className="w-full bg-[#849ebf] rounded-2xl overflow-hidden flex flex-col font-sans border border-white/10 shadow-xl">
      ...
      <div className="p-4 space-y-4"><div className="flex items-start gap-2"><div className="w-7 h-7 rounded-full bg-[#273246] flex items-center justify-center text-[9px] font-bold text-white flex-shrink-0 shadow-sm"><Smartphone size={14}/></div><div className="flex-1 w-full overflow-hidden flex flex-col items-start gap-2"><BubbleContent data={data} /></div></div></div>
    </div>
  );
}
...
export function InteractiveSimulator({ onPathUpdate }: { onPathUpdate: (path: {nodes: string[], edges: string[]}) => void }) {
    ...
    const handleSend = async (text: string) => {
        ...
        if (!targetNode) { targetNode = nodes.find(n => n.nodeName === '預設回覆'); if (targetNode) tempNodes.push(targetNode.id); }
        ...
        while(renderNode && renderNode.messageType === 'time_router' && loopCount < 5) {
            ...
            if (!config.forceOffHours) { if (currentTwTimeStr >= (config.startTime||"09:00") && currentTwTimeStr <= (config.endTime||"18:00")) { if (!config.workDays || config.workDays.includes(now.getDay())) isBusiness = true; } }
            const handle = isBusiness ? 'business' : 'off-hours';
            const edge = edges.find(e => e.source === renderNode.id && e.sourceHandle === handle);
            if (edge) { tempEdges.push(edge.id); renderNode = nodes.find(n => n.id === edge.target); if (renderNode) tempNodes.push(renderNode.id); } else { renderNode = null; }
        }
        if (renderNode && renderNode.messageType !== 'time_router') {
            ...
            setTimeout(() => { setMessages(prev => [...prev, { id: Date.now().toString(), sender: 'bot', data: renderNode }]); }, 600);
        }
    };
    ...
    const resetSimulation = () => {
        ...
        if (defaultNode) {
            setTimeout(() => {
                const initPath = { nodes: [defaultNode.id], edges: [] };
                // 🚀 重置時將路徑長度歸一，FlowEditor 的 prevPathLengthRef 會感應到
                setPathTracker(initPath); 
                onPathUpdate(initPath);
                setMessages([{ id: Date.now().toString(), sender: 'bot', data: defaultNode }]);
            }, 300);
    ...
    return (
        <div className="w-full h-full flex flex-col bg-[#849ebf] font-sans">
            <div className="h-16 bg-[#273246] flex items-center justify-between px-5 shadow-md z-10"><div className="flex items-center gap-3"><MessageCircle className="text-[#06C755]" size={20} /><span className="text-white font-black tracking-widest text-sm">LINE 實機互動模擬</span></div><button onClick={resetSimulation} className="text-slate-400 hover:text-white transition-colors bg-white/5 p-2 rounded-full" title="重新開始"><RotateCcw size={16} /></button></div>
            <div ref={scrollRef} className="flex-1 overflow-y-auto p-4 space-y-6 scrollbar-hide">
                {messages.map(msg => (
                    <div key={msg.id} className={`flex w-full ${msg.sender === 'user' ? 'justify-end' : 'justify-start'}`}>
                        {msg.sender === 'user' ? (
                            <div className="bg-[#A0F080] text-slate-800 px-4 py-2.5 rounded-2xl rounded-tr-sm text-sm shadow-sm font-medium max-w-[80%] break-words">{msg.text}</div>
                        ) : (
                            <div className="flex items-start gap-2 max-w-[90%]"><div className="w-8 h-8 rounded-full bg-[#273246] flex items-center justify-center text-[10px] font-bold text-white flex-shrink-0 shadow-sm mt-1">BOT</div><div className="flex flex-col gap-2"><BubbleContent data={msg.data} onAction={(txt) => handleSend(txt)} /></div></div>
                        )}
                    </div>
                ))}
            </div>
            <div className="bg-white p-3 border-t border-gray-200 flex gap-2 items-center pb-6">
                <input value={inputText} onChange={e => setInputText(e.target.value)} onKeyDown={e => e.key === 'Enter' && handleSend(inputText)} placeholder="輸入測試文字..." className="flex-1 bg-gray-100 rounded-full px-4 py-2.5 text-sm outline-none text-slate-700 placeholder:text-gray-400" />
                <button onClick={() => handleSend(inputText)} disabled={!inputText.trim()} className="bg-[#06C755] text-white p-2.5 rounded-full hover:brightness-110 disabled:opacity-50 disabled:cursor-not-allowed transition-all"><Send size={18} /></button>
            </div>
        </div>
    );
}
