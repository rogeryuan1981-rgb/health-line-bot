...
export function InteractiveSimulator({ onPathUpdate }: { onPathUpdate: (path: {nodes: string[], edges: string[]}) => void }) {
    ...
    useEffect(() => {
        ...
        const unsubEdges = onSnapshot(collection(db, "flowEdges"), (snap) => {
            const eData = snap.docs.map(d => ({ id: d.id, ...d.data() }));
            setEdges(eData);
        });
    ...
    const handleSend = async (text: string) => {
        ...
        while(renderNode && renderNode.messageType === 'time_router' && loopCount < 5) {
            ...
            const currentTwTimeStr = `${String(twDate.getHours()).padStart(2, '0')}:${String(twDate.getMinutes()).padStart(2, '0')}`;
            
            let isBusiness = false;
            if (!config.forceOffHours) {
                if (currentTwTimeStr >= (config.startTime||"09:00") && currentTwTimeStr <= (config.endTime||"18:00")) {
                    if (!config.workDays || config.workDays.includes(twDate.getDay())) isBusiness = true;
                }
            }
