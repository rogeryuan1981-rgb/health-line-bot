...
export default function FlowEditor() {
        ...
        return {
          ...
          data: { label: (
            <div className="flex flex-col items-center justify-center w-full h-full relative">
              ...
              {/* 👉 核心：左下角自定義標籤 (藍色系) */}
              {data.customLabel && (
                <div className="absolute bottom-1.5 left-1.5 px-2 py-0.5 rounded-md text-[9px] font-black uppercase tracking-tight bg-blue-500/20 text-blue-400 border border-blue-500/30 shadow-sm max-w-[85px] truncate">
                  {data.customLabel}
                </div>
              )}

              {/* 👉 右下角訊息類別 (原本的標籤) */}
              <div className={`absolute bottom-1.5 right-1.5 px-2 py-0.5 rounded-md text-[9px] font-black uppercase tracking-wider border shadow-sm ${
                isStart 
  ...
  const addNewNode = async () => { 
    await addDoc(collection(db, "flowRules"), { nodeName: "新關鍵字", customLabel: "", messageType: "text", position: { x: 200, y: 200 }, updatedAt: serverTimestamp() }); 
  };

  const onConnect = useCallback(async (params: Connection) => {
    if (!params.source || !params.target) return;
    await addDoc(collection(db, "flowEdges"), { source: params.source, target: params.target, sourceHandle: params.sourceHandle, targetHandle: params.targetHandle, color: '#deff9a', strokeWidth: 2, dashed: true, arrowDirection: 'forward', pathType: 'smoothstep', createdAt: serverTimestamp() });
  }, []);
  ...
  return (
    <div className="w-full h-full relative bg-[#020617] flex overflow-hidden">
      <div className="absolute top-8 left-8 z-10 flex flex-col gap-3">
          ...
          <button onClick={() => setSnapToGrid(!snapToGrid)} className={`px-4 py-2 rounded-xl text-xs font-bold tracking-widest flex items-center justify-center gap-2 transition-all border ${snapToGrid ? 'bg-slate-800 text-[#deff9a] border-[#deff9a]/30 shadow-lg' : 'bg-slate-900/50 text-slate-500 border-transparent hover:bg-slate-800'}`}><Magnet size={14} /> 磁吸對齊 {snapToGrid ? 'ON' : 'OFF'}</button>
      </div>
      <ReactFlow nodes={nodes} edges={edges} nodeTypes={nodeTypes} onNodesChange={(c) => setNodes(s => applyNodeChanges(c, s))} onEdgesChange={(c) => setEdges(s => applyEdgeChanges(c, s))} onConnect={onConnect} onNodesDelete={useCallback(async (dn: Node[]) => { for (const n of dn) await deleteDoc(doc(db, "flowRules", n.id)); }, [])} onEdgesDelete={useCallback(async (de: Edge[]) => { for (const e of de) await deleteDoc(doc(db, "flowEdges", e.id)); }, [])} onNodeClick={(_, n) => { setSelectedId(n.id); setActivePanel('node'); }} onEdgeClick={(_, e) => { setSelectedId(e.id); setActivePanel('edge'); }} onPaneClick={handlePaneClick} onNodeDragStop={async (_, n) => { await updateDoc(doc(db, "flowRules", n.id), { position: n.position }); }} connectionMode={ConnectionMode.Loose} deleteKeyCode={["Backspace", "Delete"]} snapToGrid={snapToGrid} snapGrid={[20, 20]} fitView>
