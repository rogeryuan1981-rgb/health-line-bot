  ...
  return (
    <div className="w-full h-full relative flex overflow-hidden">
      <div className="absolute top-4 left-4 z-10">
        <button onClick={addNewNode} className="bg-[#06C755] text-white px-4 py-2 rounded-md shadow-lg flex items-center gap-2 font-bold transition-all hover:scale-105">
          <Plus size={18} /> 新增節點
        </button>
      </div>
      ...
      <div className={`absolute right-0 top-0 h-full transition-transform duration-300 z-20 ${isPanelOpen ? 'translate-x-0' : 'translate-x-full'}`}>
        <div className="h-full" onClick={(e) => e.stopPropagation()}>
          <NodeEditPanel 
            ...
            onDataChange={(data: any) => setSimulatorData(data)} // 👉 修正 3：加上 any 類型
          />
        </div>
      </div>
    </div>
