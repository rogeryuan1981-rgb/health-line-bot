...
export default function FlowEditor() {
  ...
  const onConnect = async (p: any) => { 
    ...
    await addDoc(collection(db, "flowEdges"), { source: p.source, target: p.target, createdAt: serverTimestamp() }); 
  };

  return (
    <div className="w-full h-full relative flex overflow-hidden bg-background">
      <div className="absolute top-4 left-4 z-10">
        <button onClick={addNewNode} className="bg-primary text-primary-foreground px-4 py-2 rounded-md shadow-lg flex items-center gap-2 hover:opacity-90 transition-opacity font-medium">
          <Plus size={18} /> 新增節點
        </button>
      </div>
      ...
      <div className="flex-1 h-full">
        <ReactFlow 
          ...
          onPaneClick={() => { setIsPanelOpen(false); setSimulatorData({}); }} 
          fitView
        >
      ...
      <div className={`absolute right-0 top-0 h-full transition-transform duration-300 ease-in-out z-20 ${isPanelOpen ? 'translate-x-0' : 'translate-x-full'}`}>
        <div className="h-full" onClick={(e) => e.stopPropagation()}>
          <NodeEditPanel 
            ...
            onDataChange={(data) => setSimulatorData(data)} 
          />
        </div>
      </div>
